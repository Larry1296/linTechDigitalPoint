...
def test_public_catalog_never_leaks_cost_or_shelves():
    ...
    text=response.content.decode(); assert "selling_price" in text; assert "unit_cost" not in text and "shelf" not in text and "cogs" not in text and "sku" not in text and "barcode" not in text
@pytest.mark.django_db
