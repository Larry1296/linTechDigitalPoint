...
The Owner chooses a zone, records the stack's real position and dimensions in feet (`ft`), defines each level's compartment count, previews the full rack, and creates it transactionally. Codes such as `RIGHT-R01-L03-S02` are permanent; friendly names and physical sticker labels remain editable.
