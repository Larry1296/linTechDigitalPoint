...
class Command(BaseCommand):
    ...
    def handle(self,*args,**options):
        ...
        username=options["username"] or (config("OWNER_USERNAME", default="larry") if noinput else input("Username [larry]: ").strip() or "larry")
        email=options["email"] or (config("OWNER_EMAIL", default="") if noinput else input("Email: ").strip())
        first=options["first_name"] or (config("OWNER_FIRST_NAME", default="") if noinput else input("First/display name: ").strip())
        ...
        if change_password:
            if noinput:
                password=config("OWNER_PASSWORD", default="")
